...
select concat(e1.first_name,' ',e1.last_name) as 'Name', e1.job_id, e1.salary, e1.department_id, 
				(select avg(e2.salary*12) 
				from employees e2
				where e1.department_id = e2.department_id 
				group by e2.department_id) as 'Department Avg Salary'
from employees e1;				
...
select last_name, salary from employees where last_name in ('Kochhar', 'kochhar');
...
select department_id , min(salary) from employees group by department_id order by min(salary) desc;
...
from employees e2
where salary in (select min(salary) from employees e1 where e2.job_id = e1.job_id group by e1.job_id)
order by e2.job_id;

select min(salary) from employees e1 group by e1.job_id;
...
select e2.last_name,  e2.job_id, d.department_name, l.street_address
from employees e2, departments d, locations l
where
	e2.department_id = d.department_id and d.location_id = l.location_id and
	e2.employee_id in (select employee_id 
						from employees e1 
                        where job_id = 'SA_MAN' and e1.employee_id = e2.employee_id);
    

    
-- 문제 12] 가장 많은 부하직원을 갖는 MANAGER의 사원번호와 이름을 출력하시오
select emp2.employee_id, emp2.last_name
from employees emp2
where emp2.employee_id =( select manager_id
							from employees e
							group by manager_id
